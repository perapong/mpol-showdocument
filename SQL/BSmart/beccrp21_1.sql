--create table perapong.zp_bwccrp21 as
...
,      REP.REPLOC_CODE                                                  LOC
,      DECODE(NVL(REP.REPAPPT_TYPE,'0'),NVL(2,'2'),'N',NULL) FARIS 
...
SELECT INF.OU_CODE
        ...
        FROM (  SELECT OHDOU_CODE      OU_CODE
                     ...
                     , SUM(
                           CASE WHEN (NVL(OHDTRANS_GROUP,'!') IN (11,12)) AND (OHDTRANS_DATE BETWEEN TRUNC(NVL(MS4.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND TRUNC(NVL(MS3.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+0.99999) THEN   
                                NVL(TSDDUE_AMT,0)
                           ELSE 0 END 
                          ) PV3_AMT 
                     , SUM(
                           CASE WHEN (NVL(OHDTRANS_GROUP,'!') IN (11,12)) AND (OHDTRANS_DATE BETWEEN TRUNC(NVL(MS3.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND TRUNC(NVL(MS2.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+0.99999) THEN   
                                NVL(TSDDUE_AMT,0)
                           ELSE 0 END 
                          ) PV2_AMT 
                     , SUM(
                           CASE WHEN (NVL(OHDTRANS_GROUP,'!') IN (11,12)) AND (OHDTRANS_DATE BETWEEN TRUNC(NVL(MS2.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+0.99999) THEN   
                                NVL(TSDDUE_AMT,0)
                           ELSE 0 END 
                          ) PV1_AMT 
                ...
                AND    TSDYEAR        = OHDYEAR
                AND    TSDTRANS_TYPE  IN ('IIVBL') --, :TR_TYPE)
                ...
                AND    decode(OHDNSM,8,4,OHDNSM) BETWEEN NVL(:S_NSM_CODE,'!') AND NVL(:E_NSM_CODE,CHR(250))  
...
,      (SELECT CURR.OU_CODE
        ...
        FROM (
            SELECT OHDOU_CODE      OU_CODE
                 ...
                 , OHDTRANS_DATE
                 , SUM(
                       CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 11) AND (TSDBRAND_CODE = '1') THEN
                                CASE WHEN TSDTRANS_TYPE = 'IIVBL' THEN
                                     ROUND(NVL(TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                                ELSE NVL(TSDDUE_AMT,0) END
                       ELSE 0 END 
                       ) M_AMT_B1
                 , SUM(
                       CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 11) AND (TSDBRAND_CODE = '2') THEN
                                CASE WHEN TSDTRANS_TYPE = 'IIVBL' THEN
                                     ROUND(NVL(TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                                ELSE NVL(TSDDUE_AMT,0) END
                       ELSE 0 END 
                 ...
                 , SUM(
                       CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 11) AND (TSDBRAND_CODE = '3') THEN
                                CASE WHEN TSDTRANS_TYPE = 'IIVBL' THEN
                                     ROUND(NVL(TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                                ELSE NVL(TSDDUE_AMT,0) END
                       ELSE 0 END 
            ...
            SELECT TSH.TSHOU_CODE       OU_CODE
                 ...
                 , SUM(
                       CASE WHEN (TSD.TSDBRAND_CODE = '1') THEN
                            CASE WHEN TSD.TSDTRANS_TYPE = 'IRVBL' THEN
                                 -ROUND(NVL(TSD.TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                            ELSE -NVL(TSD.TSDDUE_AMT,0) END
                       ELSE 0 END 
                       ) M_AMT_B1
                 , SUM(
                       CASE WHEN (TSD.TSDBRAND_CODE = '2') THEN
                            CASE WHEN TSD.TSDTRANS_TYPE = 'IRVBL' THEN
                                 -ROUND(NVL(TSD.TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                            ELSE -NVL(TSD.TSDDUE_AMT,0) END
                       ELSE 0 END 
                 ...
                 , SUM(
                       CASE WHEN (TSD.TSDBRAND_CODE = '3') THEN
                            CASE WHEN TSD.TSDTRANS_TYPE = 'IRVBL' THEN
                                 -ROUND(NVL(TSD.TSDDUE_AMT,0)*100/(100+NVL(7,0)),2)
                            ELSE -NVL(TSD.TSDDUE_AMT,0) END
                       ELSE 0 END 
            ...
            ,      DB_MAILPLAN_SALES MS1
            WHERE  TSH.TSHTRANS_TYPE    IN ('IRVBL' , 'IRVTB')
            AND    TSH.TSHOU_CODE      = '000'
            ...
            AND    NOT EXISTS (SELECT  'X'
                               ...
                               AND     SIV.OHDTRANS_GROUP IN (11,12) 
        ...
        ,      AR_TRANSACTIONS_DTL TSD
        ,      DB_MAILPLAN_SALES MS1
        WHERE  TSH.TSHTRANS_TYPE    IN ('CCNWB' , 'CCNBL')
        AND    TSH.TSHOU_CODE      = '000'
        AND    DECODE("TSHBD_TRAN_FLAG",2,"TSHREF_REP_CODE","TSHUPD_REP_CODE")  BETWEEN NVL(:S_REP_CODE,'!') AND NVL(:E_REP_CODE,CHR(250))
        AND    TSH.TSHAR_TYPE      = '2'
        AND    TSH.TSHDOC_DATE     BETWEEN TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND TRUNC(TO_DATE(:S_DATE,'DD/MM/RRRR'))+0.99999
...
/*  CASH SALE NOT USE 13/01/2014 BY PERAPONG
        ...
        SELECT OHDOU_CODE      OU_CODE
             ...
             , 0 M_AMT_B3
             , SUM(
                   CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 12)  AND (ORBBRAND = '1') THEN
                        NVL(ORBNTB,0)
                   ELSE 0 END 
                   ) C_AMT_B1
             , SUM(
                   CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 12)  AND (ORBBRAND = '2') THEN
                        NVL(ORBNTB,0)
                   ELSE 0 END 
                   ) C_AMT_B2
             , SUM(
                   CASE WHEN TRUNC(OHDTRANS_DATE) >= TRUNC(NVL(MS1.MPSSHIPDATE,TO_DATE('01/01/0001','DD/MM/RRRR')))+1 AND (OHDTRANS_GROUP = 12)  AND (ORBBRAND = '3') THEN
                        NVL(ORBNTB,0)
                   ELSE 0 END 
                   ) C_AMT_B3
             , 0  RTN_AMT  
...
*/
        ...
        SELECT ORBOU_CODE           OU_CODE
             ...
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '11' AND ORBBRAND = '1' THEN -ORBNTB
                    ELSE 0 END
               )                    M_AMT_B1
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '11' AND ORBBRAND = '2' THEN -ORBNTB
                    ELSE 0 END
               )                    M_AMT_B2
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '11' AND ORBBRAND = '3' THEN -ORBNTB
                    ELSE 0 END
               )                    M_AMT_B3
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '12' AND ORBBRAND = '1' THEN -ORBNTB
                    ELSE 0 END
               )                    C_AMT_B1
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '12' AND ORBBRAND = '2' THEN -ORBNTB
                    ELSE 0 END
               )                    C_AMT_B2
             , SUM(
                    CASE WHEN SUBSTR(ORBINV_NO,1,2) = '12' AND ORBBRAND = '3' THEN -ORBNTB
                    ELSE 0 END
               )                    C_AMT_B3
             , 0                    RTN_AMT
        FROM   OM_ORDER_HDRG     
        ,      DB_MAILPLAN_SALES 
        WHERE  ORBOU_CODE      = '000'
...
--and    rep.reprep_code = '8350002603'
...
AND    (SELECT NVL(APPT_STATUS,'PP') APPT_STATUS FROM MS_REP_BY_BRAND REB 
        WHERE REB.OU_CODE      = REP.REPOU_CODE
          AND REB.BRAND_GROUP  = :P_BRAND 
          AND REB.REP_SEQ      = REP.REPREP_SEQ) <> 'RE'
...
AND    TRUNC(MPLSHIPDATE) = TRUNC(TO_DATE('22/01/2014','DD/MM/RRRR'))
