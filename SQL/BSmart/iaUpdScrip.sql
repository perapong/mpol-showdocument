...
-- ******* Alter Table ******* --
--insert into bw_riupd_script
select sysdate LAUNCH_DATE, '*' LOC_DESTINATION, 'alter table regprofile add phoneno VARCHAR' EXEC_SCRIPT
, 1 INACTIVE, sysdate CRE_DATE, 'admin' CRE_BY, null UPD_DATE, null UPD_BY from dual


select * from bw_riupd_script order by launch_date desc
